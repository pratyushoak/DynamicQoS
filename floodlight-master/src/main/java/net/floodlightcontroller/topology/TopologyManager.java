...
public class TopologyManager implements IFloodlightModule, ITopologyService,
		IRoutingService, ILinkDiscoveryListener, IOFMessageListener {
	 ...
	 * Debug Counters
	 */
	protected static final String PACKAGE = TopologyManager.class.getPackage()
			.getName();
	...
	protected void handleMiscellaneousPeriodicEvents() {
		...
		// can store current delay and congestion measure in the
...
//		}
		...
		for (Link link : link_bytes.keySet()) {
			...
			if (link_bytes_prev == null) {
				byte_prev = 0;
			} else {
				byte_prev = link_bytes_prev.get(link);
			}
	...
	protected boolean createNewInstance(String reason) {
		...
		TopologyInstance nt = new TopologyInstance(switchPorts, blockedPorts,
				openflowLinks, broadcastDomainPorts, tunnelPorts);
		nt.compute();
